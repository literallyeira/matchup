...
export default function IstatistiklerPage() {
  ...
  if (!selectedCharacter) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#0a0a0a] text-white gap-4">
        <p className="text-gray-400">Önce bir karakter seçmelisiniz.</p>
        <Link href="/" className="text-pink-400 hover:text-pink-300 underline">Ana Sayfaya Dön</Link>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      <div className="max-w-lg mx-auto px-4 py-12">
        ...
        {badges.length > 0 && (
          <div className="mb-8">
            <h2 className="text-sm font-semibold text-gray-400 mb-3 uppercase tracking-wider">Rozetlerim</h2>
        ...
        {stats && (
          <>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              <StatCard icon="fa-eye" label="Toplam Görüntülenme" value={stats.totalViews} color="text-blue-400" />
              <StatCard icon="fa-eye" label="Bu Hafta" value={stats.weeklyViews} color="text-cyan-400" />
              <StatCard icon="fa-heart" label="Alınan Beğeni" value={stats.likesReceived} color="text-pink-400" />
              <StatCard icon="fa-paper-plane" label="Gönderilen Beğeni" value={stats.likesSent} color="text-purple-400" />
              <StatCard icon="fa-heart-circle-check" label="Eşleşme" value={stats.matches} color="text-green-400" />
              <StatCard icon="fa-percent" label="Eşleşme Oranı" value={`${stats.matchRate}%`} color="text-yellow-400" />
            </div>

            {/* Ek bilgiler */}
            <div className="bg-white/5 border border-white/10 rounded-2xl p-5 space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-gray-400 text-sm">Üye Olma Tarihi</span>
                <span className="text-white text-sm font-medium">{formatDate(stats.memberSince)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-gray-400 text-sm">Aktif Gün Sayısı</span>
                <span className="text-white text-sm font-medium">{stats.daysActive} gün</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-gray-400 text-sm">Gönderilen Dislike</span>
